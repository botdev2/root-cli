...
internal sealed class InteractiveShell
{
    ...
    private bool OpenChatsMenu(int max = 40, string title = "All saved chats")
    {
        while (true)
        {
            ...
            Console.WriteLine();
            TermUi.Hint("Stored in %LOCALAPPDATA%\\root-cli\\sessions\\");
            Console.WriteLine();
    ...
    private bool ShowGuide()
    {
        ...
        Console.WriteLine("  3. A repository folder selected (option 4)");
        Console.WriteLine("  4. Optional MCP: desktop /root tools install codebase-memory-mcp");
        Console.WriteLine();
        Console.WriteLine("  From a terminal you can also call commands directly:");
        Dim("    RootCli.exe ask \"What is this?\" -r C:\\path\\to\\repo");
        Console.WriteLine();
        Dim("    RootCli.exe plan \"…\" -r C:\\path\\to\\repo");
        Console.WriteLine();
        Dim("    RootCli.exe agent \"…\" -r C:\\path\\to\\repo --yes");
        Console.WriteLine();
        Console.WriteLine("  Config lives under %LOCALAPPDATA%\\root-cli\\");
        Pause();
        return true;
    }
    ...
    private void DrawFarewell()
    {
        Console.CursorVisible = true;
        Console.WriteLine();
        Dim("  Bye — run RootCli.exe anytime, or pass a command for scripting.");
        Console.WriteLine();
    }
